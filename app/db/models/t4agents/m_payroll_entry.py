...
class PayrollEntryDB(Base, BaseMixin):
    ...
    schedule_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA_TOO_T4AGENTS}.payroll_schedules.id"), nullable=False, index=True,)
    payroll_period_id: Mapped[UUID | None] = mapped_column(
        ForeignKey(f"{SCHEMA_TOO_T4AGENTS}.payroll_periods.id"),
        nullable=True,
        index=True,
    )
    employee_id: Mapped[UUID] = mapped_column(ForeignKey(f"{SCHEMA_TOO_T4AGENTS}.employees.id"), nullable=False, index=True,)
